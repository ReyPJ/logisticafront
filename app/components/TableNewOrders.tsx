...
const NexOrdersTab: React.FC<OrdersProps> = ({ status }) => {
  ...
  useEffect(() => {
    ...
    const fetchSavedOrders = async () => {
      try {
        ...
        const savedOrderIds = response.data.map((order: { order_id: any; }) => order.order_id);
  ...
  const printOrder = (order: Order) => {
    ...
    if (api.defaults && api.defaults.baseURL) {
      qrCodeUrl += encodeURIComponent(
        api.defaults.baseURL + "orders/" + order.order_id + "/prepared/",
      );
    } else {
    ...
    if (printWindow) {
      ...
      alert(
        "No se pudo abrir la ventana de impresión. Asegúrate de permitir ventanas emergentes.",
      );
    }
  };
          ...
          {orders.map((order) => (
            ...
            >
              ...
              {order.status === "new" && (
                <div className="mb-4 md:pb-6 sm:pb-10 pb-12">
                  <QRCodeSVG
                    value={`${api.defaults.baseURL}orders/${order.order_id}/prepared/`}
                    size={60}
                  />
                </div>
              )}
